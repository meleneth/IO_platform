...
wchar_t *io_get_err_str(DWORD err) {
  ...
  if (FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                     ...
                     lpMsgBuf, 1024, nullptr) == 0) {
    MessageBoxW(nullptr, L"FormatMessageW failed", L"Error", MB_OK);
    ExitProcess(err);
  }
...
char *io_wchar_utf8(NATIVE_STRING_TYPE *src) {
  ...
  int count = WideCharToMultiByte(CP_UTF8, 0, src, -1, buff, MAX_PATH, nullptr,
                                  &used_default_char);
  ...
  if (used_default_char) {
    printf("io_wchar_utf8() : Used a default char in conversion.\n");
  }
...
wchar_t *io_utf8_wchar(const char *src) {
  ...
  int count = MultiByteToWideChar(
      CP_UTF8, 0, src,
      -1, // -1, the function processes the entire input string, including the
...
int io_strncasecmp(NATIVE_STRING_TYPE *wstr1, NATIVE_STRING_TYPE *wstr2,
                   int num_char) {
  return CompareStringEx(LOCALE_NAME_USER_DEFAULT, LINGUISTIC_IGNORECASE, wstr1,
                         num_char, wstr2, num_char, nullptr, nullptr, 0) -
         2;
}
...
/*int io_strncasecmp(NATIVE_STRING_TYPE *wstr1, NATIVE_STRING_TYPE *wstr2,
  ...
  return (CompareStringEx(LOCALE_NAME_USER_DEFAULT, LINGUISTIC_IGNORECASE,
                          wstr1, -1, wstr2, -1, NULL, NULL, NULL) -
          2
...
// Yes officer this overload right here TODO: kill with fire
int io_strncasecmp(const char *str1, const char *str2, int num_char) {
  return _strnicmp(str1, str2, num_char);
}
...
void *io_open_dir(char *dir_name) {
  ...
  dir->hnd = FindFirstFileW(str, &dir->dir_struct);
...
char *io_scan_dir(void *dir_stream) {
  ...
  HANDLE hnd = ptr->hnd;
  WIN32_FIND_DATAW data = ptr->dir_struct;

  int success = FindNextFileW(hnd, &data);
  if (success == 0) {
    // TODO: log to file
    DWORD err = GetLastError();
...
// return 0 == match, <0 == less than match, >0 == greater than match
int io_strncmp(NATIVE_STRING_TYPE *str1, NATIVE_STRING_TYPE *str2,
               int num_char) {
  return strncasecmp(str1, str2, num_char);
}
